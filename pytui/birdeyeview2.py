...
class BirdEyeView2(QWidget):
    ...
    def paintEvent(self, e):
        ...
        qp.fillRect(self.canvas.rect(),Qt.blue)
    ...
    def draw_grid(self, qp):
        ...
        # qp.drawLine(self.M2P(0, -800), self.M2P(0, 800)) #중심x선
        qp.drawLine(self.M2P(-800, 0), self.M2P(800, 0)) #중심 y선
        
    def target_lane(self,qp):
        qp.setPen(QPen(Qt.blue, 1)) 
        qp.drawLine(self.M2P(0.3, -800), self.M2P(0.3, 800)) #앞범퍼 끝단 선
        qp.drawLine(self.M2P(-6, -800), self.M2P(-6, 800)) #뒷차 범처 끝단
        ...
        qp.setPen(QPen(QColor('#4A4A4A'), 1)) # 좌표선 
        qp.drawLine(self.M2P(-800, -1), self.M2P(800, -1)) # 자차와 1m
    ...
    def draw_lane(self, qp,laneVal): #차선
        ...
        for lane in laneVal:
            for x1 in list(np.arange(-800, 800, resolution)):
                x2 = x1 + resolution
